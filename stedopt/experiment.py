...
class Experiment():
    ...
    The `Experiment` object implements `acquire` and `acquire_all` methods. 
    
    The `acquire_all` method allows multiprocessing. The `acquire_all` method should
    ...
    def acquire(self, name, num_acquisition, bleach=True, verbose=False, **kwargs):
        """
        ...
        :param verbose: A `bool` whether this function is verbosed
        ...
        for i in trange_(num_acquisition, leave=False, desc="[----] Acquire"):
            ...
            acquisition, bleached, other = self.microscopes[name].get_signal_and_bleach(self.datamaps[name], self.datamaps[name].pixelsize, **self.params[name],
                                                                                bleach=bleach, update=True, **kwargs)
    ...
    def acquire_all(self, num_acquisition, bleach=True, processes=0, verbose=False, **kwargs):
        """
        ...
        if processes:
            ...
            calls = [pool.apply_async(self.acquire, kwds={"name":name, "num_acquisition":num_acquisition, "bleach":bleach, "verbose":verbose, **kwargs}, callback=log_callback)
            ...
            for name in self.microscopes.keys():
                name, out = self.acquire(name=name, num_acquisition=num_acquisition, bleach=bleach, verbose=verbose, **kwargs)
                history[name] = out
        return history
