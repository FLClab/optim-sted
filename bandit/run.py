...
def run_trial(
    no_trial, config, pref_articulator, pareto_sampler, experiment_generator,
    knowledge_generator, routine_generator, verbose=False
):
    """
        ...
        try:
            with h5py.File(os.path.join(config.save_folder, "optim.hdf5"), "a") as logging:
                ...
                for dir_name in ["conf1", "conf2"]:
                    create_dataset(logging[dir_name], str(no_trial), shape=(config.optim_length, *datamap_generator.shape), dtype=numpy.uint16, pixelsize=config.default_values_dict["pixelsize"])
                ...
                for dir_name in config.im_dir_names[-3:]:
    ...
    for iter_idx in range(config.optim_length):
        ...
        t0 = time.time()
        X, y_samples, points_arr2d, timesperpixel, ndf = pareto_sampler(iter_idx, algos, config.with_time, conditions=config.conditions)
        ...
        # Acquire STED image
        name, sted_history = experiment.acquire("STED", config.default_values_dict["num_acquisition"], bleach=True, verbose=False)
        sted_image = sted_history["acquisition"]
        ...
        while not saved:
            try:
                ...
                with h5py.File(os.path.join(config.save_folder, "optim.hdf5"), "a") as logging:
                    ...
                    logging["other"][str(no_trial)][iter_idx] = sted_history["other"][-1]
            except (BlockingIOError, OSError) as err:
                # print(err)
                time.sleep(random.random() * 0.2)

        sted_image = sted_image[-1] # only keeps last sted image
        ...
        for name in config.obj_names:
            if name == "Resolution":
                ...
                valid = sted_history["other"][-1] >= 1
                acquisition = sted_image.copy()
                acquisition = sted_history["acquisition"][0] # Evaluates on first image
                value = obj_dict[name].evaluate([acquisition], conf1, conf2, fg_s, fg_c, positions=positions)
                if value == obj_dict[name].res_cap:
                    ...
                    value = objectives.FWHMResolution(20e-9).evaluate([acquisition], conf1, conf2, fg_s, fg_c, positions=positions)
            elif name == "Squirrel":
                threshold = numpy.quantile(conf1, 0.99)
                fg = conf1 > threshold
                value = obj_dict[name].evaluate([sted_image], conf1, conf2, fg_s, fg_c)
            else:
                value = obj_dict[name].evaluate([sted_image], conf1, conf2, fg_s, fg_c)
        ...
        t0 = time.time()
        # weights = [1.0 if y_result[0].item() < 225 else 0.01, 1.0, 1.0]
        ...
        [
            algos[i].update(
                x_selected.T, y_result[i].flatten(), weights=[weights[i]],
                update_posterior=update_posterior
            ) for i in range(len(config.obj_names))
        ]
...
def run_TS(config, prefart="random", restore_folder=None, dry_run=False):
    ...
    func = partial(run_trial,
        ...
        routine_generator=routine_generator, verbose=config.multiprocess_opts["verbose"]
    )
