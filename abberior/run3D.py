...
def run_TS(config, prefart="random", restore_folder=None, dry_run=False, verbose=True):
    ...
    if isinstance(restore_folder, str):
        ...
        for name, conf in zip(["confocal", "sted", "overview"], [config_conf, config_sted, config_overview]):
            with open(os.path.join(save_folder, "imspector-config_{}".format(name)), "w") as f:
    ...
    for no_trial in trials:
                ...
                with h5py.File(os.path.join(save_folder, "optim.hdf5"), "a") as logging:
                    ...
                    for dir_name in ["conf1", "conf2"]:
                        create_dataset(
                            ...
                            shape=(config.optim_length, 40, *[int(config.default_values_dict["imagesize"][i] / config.default_values_dict["pixelsize"][i]) for i in range(2)]),
                            dtype=numpy.uint16
                        )
                    for dir_name in ["sted"]:
                        ...
                        create_dataset(
                            logging[dir_name], str(no_trial),
                            shape=(config.optim_length, num_channels, 40, *[int(config.default_values_dict["imagesize"][i] / config.default_values_dict["pixelsize"][i]) for i in range(2)]),
                            dtype=numpy.uint16
                        )
        ...
        for iter_idx in range(config.optim_length):
            ...
            abberior.microscope.set_scan_axes(config_conf, "xy")
            input("Manually set the position of the z-scan. Then press enter :)")

            coarse_range = abberior.microscope.get_coarse_range(config_conf)
            abberior.microscope.set_scan_axes(config_conf, "xyz")
            time.sleep(0.25)

            # Acquire a confocal image that will be used in the history
            conf1, _ = abberior.microscope.acquire(config_conf)
            time.sleep(0.25)
            conf1 = numpy.array(conf1)[0]
            conf1_slice = conf1[conf1.shape[0] // 2].copy() # Select middle slice
            fg_init = utils.get_foreground(conf1_slice)

            history = defaultdict(list)
            for n in range(config.default_values_dict["num_acquisition"]):

                # Extract context from the last acquired confocal image
                fg_c = utils.get_foreground(conf1_slice)
                context = context_handler(conf1_slice, fg_c)
                ...
                # conf1, _ = abberior.microscope.acquire(config_conf)
                # Sets the same coarse range as confocal
                time.sleep(0.25)
                abberior.microscope.set_coarse_range(config_conf, *coarse_range)
                abberior.microscope.set_coarse_range(config_sted, *coarse_range)
                abberior.microscope.set_coarse_range(config_overview, *coarse_range)
                time.sleep(0.25)

                sted_image, _ = abberior.microscope.acquire(config_sted)

                # Sets the coarse range of parameters then acquire
                time.sleep(0.25)
                abberior.microscope.set_coarse_range(config_conf, *coarse_range)
                abberior.microscope.set_coarse_range(config_sted, *coarse_range)
                abberior.microscope.set_coarse_range(config_overview, *coarse_range)
                ...
                sted_image = numpy.array(sted_image)
                conf2 = numpy.array(conf2)[0]

                sted_image_slice = sted_image[-1, sted_image.shape[1] // 2]
                conf2_slice = conf2[conf2.shape[0] // 2]
                ...
                if numpy.any(sted_image_slice):
                    fg_s = utils.get_foreground(sted_image_slice)
                else:
                ...
                for name in config.obj_names:
                    if name == "Resolution":
                        ...
                        acquisition = sted_image_slice
                        value = obj_dict[name].evaluate([acquisition], conf1_slice, conf2_slice, fg_s, fg_init)
                    elif name == "Squirrel":
                        threshold = numpy.quantile(conf1_slice, 0.95)
                        fg = conf1_slice > threshold
                        value = obj_dict[name].evaluate([sted_image_slice], conf1_slice, conf2_slice, fg_s, fg_init)
                    else:
                        value = obj_dict[name].evaluate([sted_image_slice], conf1_slice, conf2_slice, fg_s, fg_init)
