...
from stedopt.tools import create_dataset, create_group, ParameterSpaceGenerator, \
                        KnowledgeGenerator, create_savename, print_, get_start_trial, \
...
PATH = os.path.join(
    ...
    "20230502_DyMIN-optim"
)
PATH = os.path.join(os.getcwd(), "data", "20230705_STED-optim")
...
def run_TS(config, prefart="random", restore_folder=None, dry_run=False, verbose=True):
    ...
    if isinstance(restore_folder, str):
        ...
        print("[----]", save_folder)
        print("========================================================\n")
        start_trial = get_start_trial(save_folder, config.nbre_trials, config.optim_length)
    ...
    for no_trial in range(start_trial, config.nbre_trials):
        ...
        with h5py.File(os.path.join(save_folder, "optim.hdf5"), "a") as logging:
            ...
            for dir_name in ["conf1", "conf2"]:
                num_channels = abberior.microscope.get_num_channels(config_conf)
                create_dataset(
        ...
        for iter_idx in range(config.optim_length):
            ...
            conf2 = numpy.array(conf2).squeeze(axis=1)
            ...
            for n in range(config.default_values_dict["num_acquisition"]):
                ...
                conf1 = numpy.array(conf1).squeeze(axis=1)
                ...
                # Evaluate for each images in stack
                y_results = []
                for c1, s, c2, fg_i in zip(conf_init, sted_image, conf2, fg_init):

                    # foreground on confocal image
                    fg_c = utils.get_foreground(c1)
                    # foreground on sted image
                    if numpy.any(s):
                        fg_s = utils.get_foreground(s)
                    else:
                    ...
                    for name in config.obj_names:
                        if name == "Resolution":
                            ...
                            acquisition = s
                            value = obj_dict[name].evaluate([acquisition], c1, c2, fg_s, fg_i)
                        elif name == "Squirrel":
                            threshold = numpy.quantile(c1, 0.95)
                            fg = c1 > threshold
                            value = obj_dict[name].evaluate([s], c1, c2, fg_s, fg)
                        else:
                            value = obj_dict[name].evaluate([s], c1, c2, fg_s, fg_i)
                        y_result.append(value)
                    y_result = numpy.array(y_result)
                    y_results.append(y_result)
                y_result = numpy.mean(y_results, axis=0)
